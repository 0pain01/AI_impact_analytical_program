...
| connector-jira, api-core | `JIRA_BASE_URL` | Investment Profile's "view in Jira" link on the PR↔ticket verification drill-down | api-core reuses connector-jira's own var (`jira.site-base-url`) rather than a second one for the same real site — set it once, both services read it; empty → the drill-down shows the issue key with no link instead of a guessed URL |
